...
class PreProcessorConfig:
        ...
        for var, tp in variables_types.items():
            ...
            if not tasks_types.issubset(valid_preprocess[tp].keys()):
                ...
                for task_type, task_name in tsk.items():
                    ...
                    if not task_name.issubset(valid_preprocess[tp][task_type]):
                        err_msg = f"Error in preprocessing config for '{var}' with task type {task_type}."
                        err_msg += f"{task_name} is not a valid task. Valid tasks are: {valid_preprocess[tp][task_type]}"
                        logger.error(err_msg)
                        raise ValueError(err_msg)
    ...
    def from_dict(cls, d: Dict):
        variables_types = {name: info["type"] for name, info in d.items()}
